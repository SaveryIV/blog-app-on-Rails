...
RSpec.describe 'When I open user index page', type: :system do
  ...
  before :all do
    ...
    @index = 1
    (1..6).each do
      Comment.create(post: @first_post, user_id: @second_user, text: "Comment ##{@index}")
      @index += 1
      Comment.create(post: @first_post, user_id: @third_user, text: "Comment ##{@index}")
      @index += 1
    end
  ...
  it 'I can see the first comments on a post.' do
    visit "/users/#{@first_user.id}/posts?page=1"
    sleep(1)
    expect(page).to_not have_content('Lilly: Comment #9')
    expect(page).to_not have_content('Alan: Comment #10')
    expect(page).to_not have_content('Lilly: Comment #11')
    expect(page).to_not have_content('Alan: Comment #12')
  end
